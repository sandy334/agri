...
// Initialize Gemini — use Vite env variable
...
// --- Configuration: API base (frontend) ---
const API_BASE = import.meta.env.VITE_API_BASE || 'http://localhost:4000';

// --- Simulated/Remote Database Service (Auth + Storage) ---
...
export const AuthService = {
  register: async (email: string, password: string): Promise<User> => {
    ...
    try {
      ...
      // Fallback to localStorage behavior when server unavailable
      try {
  ...
  login: async (email: string, password: string): Promise<User> => {
    ...
    try {
      ...
      // Fallback to localStorage when server unreachable
      const users = JSON.parse(localStorage.getItem(USERS_STORAGE_KEY) || "[]");
